...
export const metadata: Metadata = { title: "Programs & Sessions", description: "Discover music, literature, craft, food, art and family experiences at Jashn-e-Hindustan.", openGraph: { images: ["/og.jpg"] } };
        ...
        <div className="grid gap-x-7 gap-y-14 md:grid-cols-2 lg:grid-cols-3">
          {programs.map((program, index) => <AnimatedSection key={program.title} delay={(index % 3) * .06} className={index === 0 ? "lg:col-span-2" : ""}>
            <article className="group">
              <div className={`relative overflow-hidden rounded-[1.25rem] bg-maroon ${index === 0 ? "aspect-[16/9]" : "aspect-[4/3]"}`}><Image src={program.image} alt="" fill sizes={index === 0 ? "(max-width: 1024px) 100vw, 66vw" : "(max-width: 768px) 100vw, 33vw"} className="object-cover transition-transform duration-700 group-hover:scale-105" placeholder="blur" blurDataURL={BLUR_DATA_URL} /><span className="absolute left-5 top-5 rounded-full bg-black/70 px-3 py-2 text-[.6rem] uppercase tracking-[.18em] text-gold backdrop-blur">{program.category}</span></div>
