...
export default function VisitorsGuidePage() {
  return (
    ...
    <section className="bg-cream py-20 md:py-32"><div className="section-shell">
      ...
      <AnimatedSection className="mt-24 grid overflow-hidden rounded-[1.5rem] bg-black text-cream md:grid-cols-2"><div className="relative min-h-[25rem]"><Image src="/images/qutub.jpg" alt="Delhi heritage landscape" fill sizes="(max-width: 768px) 100vw, 50vw" quality={90} className="object-cover opacity-70" placeholder="blur" blurDataURL={BLUR_DATA_URL} /><div className="absolute inset-0 bg-maroon/20" /><div className="absolute bottom-6 left-6 rounded-full bg-black/75 px-4 py-2 text-xs uppercase tracking-[.16em]"><MapPin className="mr-2 inline text-gold" size={14} />Sunder Nursery, New Delhi</div></div><div className="p-8 md:p-14"><p className="text-xs uppercase tracking-[.2em] text-gold">At the venue</p><h2 className="mt-4 font-serif text-5xl">Made for a full day</h2><ul className="mt-8 space-y-4 text-cream/70">{["Free drinking-water refill points", "Accessible paths and reserved stage viewing", "Cloakroom for small bags only", "Information and first-aid desks", "Quiet room and family care space"].map((item) => <li key={item} className="flex gap-3"><Check className="mt-1 shrink-0 text-gold" size={16} />{item}</li>)}</ul></div></AnimatedSection>
